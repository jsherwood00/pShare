...
class RegistryNode:
    ...
    def distribute(self, filename, uuids, m, k):
        ...
        try:
            if enclib.encrypt_filename(filename) is not None:
                raise Exception("Cannot upload the same file twice!")
            ...
            if not masked_filename:
                raise Exception("Error encrypting file on upload")
    ...
    def download(self, filename: str) -> bool:
        ...
        try:
            ...
            relevant_uuids = [] # all connected uuids containing a chunk
            ...
            if len(connected_uuids) < num_required:
                raise Exception(f"Not enough required snodes to reconstruct specified file: {filename}")


            # Get k random snodes from the n connected with chunks
            random_relevant_uuids = self.get_random_snodes(num_required)

            # Each relevant snode only stores 1 chunk of a file, and since
            # a file cannot be reuploaded until deleted, the chunks will
            # be unique (i.e. snode.example1 cannot have the same chunk as snode.example2)
            for uuid in random_relevant_uuids:
                stored_chunks = uuid_to_chunks[uuid]
                for stored_chunk in stored_chunks:
                    if stored_chunk[:-2] == filename:
                        relevant_chunk = stored_chunk
    ...
    def redistribute(self, file_name, uuid):
        """Given the file and S-node uuid, download the chunk from remote, delete from specified S-node,
           and re-upload if other S-nodes are available. 
           TODO future improvement: make more advanced
           i.e. if file in 6 chunks and 1 s-node with 1 chunk disconnects and none of the others are
           connected, current behavior is to delete the file, instead could store the file and future
           redistribution information in a .json and when enough of the other (if applicable) S-nodes
           reconnect, then can have the `healing` property
        """
        num_snodes = len(self.get_uuids())
        k = self.get_required(file_name) # zfec k
        if num_snodes == 1 and k == 1: # can reconstruct, but not redistribute (no S-nodes left to redistribute to)
            self.download(file_name)

            # Remote delete file
            self.delete_file(file_name)
            return 2
        # Enough s-nodes to reconstruct, and at least 1 additional S-node: download, delete, redistribute
        elif num_snodes >= k and num_snodes > 1: 
            self.download(file_name)

            # Move downloaded chunks to upload folder
            for i in range(k):
                chunk_name = file_name + f".{i}"
                download_path = os.path.join(os.getcwd(), "downloaded_files", chunk_name)
                upload_path = os.path.join(os.getcwd(), "uploading_files", chunk_name)
                if os.path.exists(download_path):
                    shutil.move(download_path, upload_path)
            
            self.delete_file(file_name)

            # Re-upload file if possible
            client_list = self.get_client_list()
            uuids = [client["uuid"] for client in client_list]
            uuids.remove(uuid)
            m = len(client_list) - 1 # 1 snode is currently being removed
            k = encrypt.genChunkNum(self, m) # new k for 1 less snode
            self.distribute(file_name, uuids, m, k)
            return 3
    ...
    def fill_storage_request(self, required_storage):
        ...
        for client in client_list:
            uuid = client["uuid"]
            used_storage = snodes_storage_used[uuid] if uuid in snodes_storage_used else 0 # S-node used storage
            storage_limit = client["storage"] # S-node tribute storage cap
