...
def encrypt_chunk(key, input_file_path, chunk_num, chunk_size):
    ...
    with open(f"temp/{file_name}_info_/{hashlib.sha512(file_name.encode()).hexdigest()}", "wb") as output:
        output.write(salt)
...
def encrypt(input_file_path: str):
    ...
    sha_basename = hashlib.sha512(os.path.basename(input_file_path).encode()).hexdigest()
    print(sha_basename)
    if p_chunk_num > 0:
...
def decrypt_chunk(key, input_file_path, chunk_num: int):
    ...
    file_name = os.path.basename(input_file_path)
    chunk_path = f"temp/{file_name}_info_/{hashlib.sha512(file_name.encode()).hexdigest()}"
...
def decrypt(input_file_path: str):
    ...
    chunk_path = f"{hashlib.sha512(file_name.encode()).hexdigest()}"
